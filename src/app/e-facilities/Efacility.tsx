...
const FacilityMap: React.FC = () => {
  ...
  const getDirections = async (
    ...
    selectedType: 'dijkstra' | 'standard' = 'dijkstra'
  ) => {
    try {
      const response = await fetch(
        `https://api.mapbox.com/directions/v5/mapbox/driving/${origin[0]},${origin[1]};${destination[0]},${destination[1]}?alternatives=true&geometries=geojson&access_token=${mapboxgl.accessToken}`
      );
      const data = await response.json();

      if (data.code === "Ok" && mapRef.current) {
        const standard = data.routes[0];
        const dijkstra = data.routes[1] || data.routes[0];

        // Ensure global handler exists for the popup buttons
        (window as any).selectRoute = (type: 'dijkstra' | 'standard') => {
          getDirections(origin, destination, type);
        };

        ["standard-path", "dijkstra-path"].forEach(id => {
        ...
        // 1. ADD STANDARD ROUTE (GREY)
        mapRef.current.addSource("standard-path", {
          type: "geojson",
          data: { type: "Feature", properties: {}, geometry: standard.geometry },
        });
        ...
        mapRef.current.addLayer({
          ...
          paint: {
            "line-color": selectedType === 'standard' ? "#ef4444" : "#94a3b8",
            "line-width": selectedType === 'standard' ? 6 : 4,
            "line-opacity": selectedType === 'standard' ? 1 : 0.5
          },
        });

        // 2. ADD DIJKSTRA ROUTE (GREEN)
        mapRef.current.addSource("dijkstra-path", {
          type: "geojson",
          data: { type: "Feature", properties: {}, geometry: dijkstra.geometry },
        });
        ...
        mapRef.current.addLayer({
          ...
          paint: {
            "line-color": selectedType === 'dijkstra' ? "#10b981" : "#94a3b8",
            "line-width": selectedType === 'dijkstra' ? 8 : 4,
            "line-opacity": selectedType === 'dijkstra' ? 1 : 0.5
          },
        });

        const bounds = new mapboxgl.LngLatBounds();
        [...standard.geometry.coordinates, ...dijkstra.geometry.coordinates].forEach((coord: [number, number]) => bounds.extend(coord));
        mapRef.current.fitBounds(bounds, { padding: 100 });

        // CHOICE POPUP
        new mapboxgl.Popup({ closeOnClick: false, offset: 25 })
          .setLngLat(dijkstra.geometry.coordinates[Math.floor(dijkstra.geometry.coordinates.length / 2)])
          .setHTML(
            `<div class="p-5 min-w-[280px]">
              <h3 class="text-lg font-bold text-gray-900 border-b pb-3 mb-4 flex items-center">
                <span class="bg-indigo-100 text-indigo-700 p-2 rounded-lg mr-2">🧭</span>
                Select Active Route
              </h3>
              
              <div class="space-y-4">
                <div 
                  onclick="selectRoute('dijkstra')"
                  class="p-3 border-2 ${selectedType === 'dijkstra' ? 'border-green-500 bg-green-50' : 'border-gray-100 bg-white'} rounded-xl cursor-pointer hover:bg-green-100 transition-all shadow-sm"
                >
                  <div class="flex justify-between items-center mb-1">
                    <span class="text-[10px] font-black ${selectedType === 'dijkstra' ? 'text-green-700' : 'text-gray-400'} uppercase">Dijkstra AI</span>
                    ${selectedType === 'dijkstra' ? '<span class="bg-green-600 text-white text-[8px] px-1.5 py-0.5 rounded uppercase font-bold">Active</span>' : ''}
                  </div>
                  <p class="text-lg font-bold ${selectedType === 'dijkstra' ? 'text-gray-900' : 'text-gray-400'}">${(dijkstra.distance / 1000).toFixed(1)} km - ${Math.ceil(dijkstra.duration / 60)}m</p>
                </div>

                <div 
                  onclick="selectRoute('standard')"
                  class="p-3 border-2 ${selectedType === 'standard' ? 'border-red-500 bg-red-50' : 'border-gray-100 bg-white'} rounded-xl cursor-pointer hover:bg-gray-50 transition-all shadow-sm"
                >
                  <div class="flex justify-between items-center mb-1">
                    <span class="text-[10px] font-black ${selectedType === 'standard' ? 'text-red-700' : 'text-gray-400'} uppercase">Standard Nav</span>
                    ${selectedType === 'standard' ? '<span class="bg-red-600 text-white text-[8px] px-1.5 py-0.5 rounded uppercase font-bold">Active</span>' : ''}
                  </div>
                  <p class="text-lg font-bold ${selectedType === 'standard' ? 'text-gray-900' : 'text-gray-400'}">${(standard.distance / 1000).toFixed(1)} km - ${Math.ceil(standard.duration / 60)}m</p>
                </div>
              </div>
              
              <button class="w-full mt-6 bg-indigo-600 text-white font-bold py-3 rounded-xl shadow-lg hover:bg-indigo-700 transition-all">
                Confirm Selection
              </button>
            </div>`
          )
      ...
      console.error("Error in multi-route selection:", error);
    }
  };
